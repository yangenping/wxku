...
namespace WeiXinLib
{
    public delegate void ApiTicketEventHandler(ApiTicketModel apiTicketModel);

    /// <summary>
    /// ApiTicket处理类(定时获取器)
    /// 每间隔指定的时间，向微信服务器请求ApiTicket。
    /// jsapi_ticket是公众号用于调用微信JS接口的临时票据。
    /// 正常情况下，jsapi_ticket的有效期为7200秒，通过access_token来获取。
    /// 由于获取jsapi_ticket的api调用次数非常有限，频繁刷新jsapi_ticket会导致api调用受限，影响自身业务，开发者必须在自己的服务全局缓存jsapi_ticket 。 
    /// </summary>
    public class ApiTicketTimer
    {
        /// <summary>
        /// ApiTicket获取事件
        /// </summary>
        public event ApiTicketEventHandler ApiTicketEvent;
        ...
        public ApiTicketTimer(string appId/*, string accessToken*/)
        {
            this.appId = appId;
